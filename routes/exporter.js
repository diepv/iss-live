...
//1. get the final list of wanted items 
		...
		if(name!==null && name!==undefined){
			backUpCollection(name).then(function(fulfilled){ // then we want to go on to copy that into our csv file
				...
				return removeFromCollection(fulfilled);
					}, function(rejected){ console.log('rejected from collectionToCsv');}).then(function(fulfilled){
						start(wanted.pop());
	...
	function getDataIntoBackup(collectionName){
		return new Promise(function(fulfill, reject){
			...
			console.log("OPENED DATA BASE issLiveData6");
	...
	function getDataIntoCSV(item, bulkRemove){
		...
		var spawn = require('child_process').spawn;
				...
				'issLiveData6','--collection','ethos','--limit','1000','--query','{Name:\"'+item+'\"}','--csv','--fields','Name,StatusClass,DataValue,DataTimeStamp','--out','dataOut/'+item+'_'+version+'.csv']);
